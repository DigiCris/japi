...
/*!
* \brief       delete all that matches shop in order
* \details     Search in the database order that matches shop and delete it if it exists.
* \param       (VARCHAR) shop The identifier of the row/rows in the order table to delete.
...
** \return • 'This order did not exist.' -> When the order shop was not found or did not exist. 
*/
function deleteByShop($shop) {
    $register = new order();
    $exists = $register->readShop($shop);

    if(count($exists) > 0) {
        $success['response'] = $register->deleteByShop($shop);
        $exists = $register->readShop($shop);
        if(count($exists) > 0) {
