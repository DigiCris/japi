...
* \details      Update the information of user table, searching it by kyc.
...
* \param      $kyc       (INT)    Identifier of the user to update. know the user
...
* \return      $success['msg'] (string) Returns a message explaining the status of the execution.
    ...
    * \return •    'We could not find the kyc you are trying to update.' -> When the user's field kyc does not exist or it is not found in the database.
*/

function updateAllByKyc($id, $username, $firstName, $lastName, $email, $password, $phone, $country, $state, $city, $rol, $kyc, $tarjeta, $cuenta, $admin, $priceKm, $zona1, $zona2, $zona3, $zona4) {
    $register = new user();

    $success['response'] = $register->readKyc($kyc);
    ...
    if($success['response']['kyc'] == $kyc) {
        ...
        $success['response'] = $register->updateAllByKyc($kyc);
        $success['response'] = $register->readKyc($kyc);
        if($success['response']['email'] == $email and $success['response']['kyc'] == $kyc) {
        ...
        $success['success'] = false;
        $success['msg'] = 'We could not find the kyc you are trying to update.';
    }
    return $success;
}
