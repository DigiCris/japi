...
* \details      Update the information of user table, searching it by rol.
...
* \param      $rol       (INT)    Identifier of the user to update. worker / shop / normal user
...
* \return      $success['msg'] (string) Returns a message explaining the status of the execution.
    ...
    * \return •    'We could not find the rol you are trying to update.' -> When the user's field rol does not exist or it is not found in the database.
*/

function updateAllByRol($id, $username, $firstName, $lastName, $email, $password, $phone, $country, $state, $city, $rol, $kyc, $tarjeta, $cuenta, $admin, $priceKm, $zona1, $zona2, $zona3, $zona4) {
    $register = new user();

    $success['response'] = $register->readRol($rol);
    ...
    if($success['response']['rol'] == $rol) {
        ...
        $success['response'] = $register->updateAllByRol($rol);
        $success['response'] = $register->readRol($rol);
        if($success['response']['zona1'] == $zona1 and $success['response']['rol'] == $rol) {
        ...
        $success['success'] = false;
        $success['msg'] = 'We could not find the rol you are trying to update.';
    }
    return $success;
}
