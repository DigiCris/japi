...
* \details      Update the information of user table, searching it by cuenta.
* \param      $id     (INT)  id to update.  1 unique id for each user
...
* \param      $cuenta       (VARCHAR)    Identifier of the user to update. wallet in app
...
* \return      $success['msg'] (string) Returns a message explaining the status of the execution.
    ...
    * \return •    'We could not find the cuenta you are trying to update.' -> When the user's field cuenta does not exist or it is not found in the database.
*/

function updateAllByCuenta($id, $username, $firstName, $lastName, $email, $password, $phone, $country, $state, $city, $rol, $kyc, $tarjeta, $cuenta, $admin, $priceKm, $zona1, $zona2, $zona3, $zona4) {
    $register = new user();

    $success['response'] = $register->readCuenta($cuenta);
    ...
    if($success['response']['cuenta'] == $cuenta) {
        ...
        $success['response'] = $register->updateAllByCuenta($cuenta);
        $success['response'] = $register->readCuenta($cuenta);
        if($success['response']['priceKm'] == $priceKm and $success['response']['cuenta'] == $cuenta) {
        ...
        $success['success'] = false;
        $success['msg'] = 'We could not find the cuenta you are trying to update.';
    }
    return $success;
}
