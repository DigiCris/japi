...
/*!
* \brief       delete all that matches kyc in user
* \details     Search in the database user that matches kyc and delete it if it exists.
* \param       (INT) kyc The identifier of the row/rows in the user table to delete.
...
** \return • 'This user did not exist.' -> When the user kyc was not found or did not exist. 
*/
function deleteByKyc($kyc) {
    $register = new user();
    $exists = $register->readKyc($kyc);

    if(count($exists) > 0) {
        $success['response'] = $register->deleteByKyc($kyc);
        $exists = $register->readKyc($kyc);
        if(count($exists) > 0) {
