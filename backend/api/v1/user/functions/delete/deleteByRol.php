...
/*!
* \brief       delete all that matches rol in user
* \details     Search in the database user that matches rol and delete it if it exists.
* \param       (INT) rol The identifier of the row/rows in the user table to delete.
...
** \return • 'This user did not exist.' -> When the user rol was not found or did not exist. 
*/
function deleteByRol($rol) {
    $register = new user();
    $exists = $register->readRol($rol);

    if(count($exists) > 0) {
        $success['response'] = $register->deleteByRol($rol);
        $exists = $register->readRol($rol);
        if(count($exists) > 0) {
