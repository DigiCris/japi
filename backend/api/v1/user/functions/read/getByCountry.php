...
/*!
* \brief       Read all that matches country in user
* \details     Search in the database user that matches country and returns them in an array.
* \param       (VARCHAR) country searching table by matching it.
...
function getByCountry($country) {
    $register = new user();
    $success['response'] = $register->readCountry($country);
