...
/*!
* \brief       Reads all user table
* \details     Search in the database all user and returns them in an array.
...
function login($user,$password) {
    ...
    $success['response'] = $register->login($user,$password);

    //$result['msg'] = 'entre a getAll dentro de la carpeta read. Ya llame a handler y sali2';
    //return $result;

    if($success['response']) {
        $success['success'] = true;
        $success['msg'] = 'users loged.';
    }else {
        $success['success'] = false;
        $success['msg'] = 'We could not loged the users.';
    }
    return $success;
}
