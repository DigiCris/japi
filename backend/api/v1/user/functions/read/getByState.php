...
/*!
* \brief       Read all that matches state in user
* \details     Search in the database user that matches state and returns them in an array.
* \param       (VARCHAR) state searching table by matching it.
...
function getByState($state) {
    $register = new user();
    $success['response'] = $register->readState($state);
